...
function decide() {
  if (!run || run.conclusion !== "failure") return { action: "none", reason: "watched run did not fail" };
  ...
  const fingerprint = createHash("sha256").update(`${run.name}|${run.head_branch}`).digest("hex").slice(0, 16);
  const marker = `<!-- facility-doctor:${fingerprint} -->`;
  const comments = JSON.parse(gh(["api", `repos/${repo}/issues/${prNumber}/comments?per_page=100`]));
  if (comments.some((c) => c.body?.includes(marker))) {
    return { action: "none", reason: "this failure fingerprint was already handled on the PR" };
  }

  const failedJobs = JSON.parse(gh(["api", `repos/${repo}/actions/runs/${run.id}/jobs?per_page=50`]))
  ...
  const files = gh(["api", `repos/${repo}/pulls/${prNumber}/files?per_page=100`, "--jq", ".[].filename"])
    .split("\n")
    .filter(Boolean);
  const sensitive = files.filter((f) => SENSITIVE_PATHS.some((p) => f === p || f.startsWith(p) || f.includes(`/${p}`)));
  const authorIsBot = (pr.user?.login ?? "").endsWith("[bot]");

  const base = { prNumber, pr, fingerprint, marker, failedJobs, sensitive };
  if (sensitive.length) return { ...base, action: "triage", reason: `PR touches sensitive surfaces (${sensitive.slice(0, 5).join(", ")})` };
  if (!authorIsBot) return { ...base, action: "triage", reason: "human-authored PR — the doctor never commits to your branch uninvited" };
  ...
  JSON.stringify(
    {
      ...
      pr: { number: decision.prNumber, headRef: decision.pr.head.ref, baseRef: decision.pr.base.ref },

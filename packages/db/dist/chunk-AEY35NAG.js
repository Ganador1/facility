...
async function seed(connectionString = process.env.DATABASE_URL) {
  ...
  try {
    ...
    for (const file of files) {
      ...
      const name = rel.replace(/^packages\/cli\/(templates|modules)\//, "").replace(extname(rel), "") || basename(file);
...
async function upsertRegistry(sql, kind, name, description, content) {

...
var registryVersions = pgTable(
  "registry_versions",
  {
    id: text("id").primaryKey(),
    orgId: text("org_id").notNull().references(() => orgs.id),
    itemId: text("item_id").notNull().references(() => registryItems.id),
...
var agentDefs = pgTable(
  "agent_defs",
  {
    ...
    contractItemId: text("contract_item_id").notNull().references(() => registryItems.id),
...
var proposals = pgTable(
  "proposals",
  {
    ...
    projectId: text("project_id").references(() => projects.id),
    runId: text("run_id").references(() => runs.id),
    actionTypeId: text("action_type_id").notNull().references(() => actionTypes.id),
...
var kbEntries = pgTable(
  "kb_entries",
  {
    id: text("id").primaryKey(),
    orgId: text("org_id").notNull().references(() => orgs.id),
    spaceId: text("space_id").notNull().references(() => kbSpaces.id),
    type: text("type").notNull(),
...
var kbLinks = pgTable(
  "kb_links",
  {
    orgId: text("org_id").notNull().references(() => orgs.id),
    spaceId: text("space_id").notNull().references(() => kbSpaces.id),
    fromEntry: text("from_entry").notNull().references(() => kbEntries.id),
    toEntry: text("to_entry").notNull().references(() => kbEntries.id),
    createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull()
  },
  (table) => [

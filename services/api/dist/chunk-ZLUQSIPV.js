...
// src/app.ts
import { can, newId as newId2, open, seal as seal2, verifyKey } from "@facility/core";
import {
  apiKeys as apiKeys2,
  createDb,
  insertAuditEvent,
...
async function registerV1Routes(app, config) {
  ...
  app.post(
    ...
    async (request) => {
      const p = principal(request);
      const { runId } = request.params;
      return (await db.update(runs).set({ status: "canceled", endedAt: /* @__PURE__ */ new Date() }).where(and(eq2(runs.orgId, p.orgId), eq2(runs.id, runId))).returning())[0];
    }
  );
  app.get(
  ...
  app.post(
    ...
    async (request) => {
      const p = principal(request);
      const { runId } = request.params;
      return (await db.insert(steerMessages).values({
        id: newId("evt"),
        orgId: p.orgId,
        runId,
...
async function resolvePrincipal(request, db, config) {
  ...
  if (auth?.startsWith("Bearer fak_")) {
    ...
    const rows = await db.select({
      key: apiKeys2,
      role: rolesTable
    }).from(apiKeys2).innerJoin(rolesTable, eq3(apiKeys2.roleId, rolesTable.id)).where(and2(eq3(apiKeys2.prefix, "fak"), isNull2(apiKeys2.revokedAt)));

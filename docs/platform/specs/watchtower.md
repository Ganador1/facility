...
`watchtower.outcomes` (nightly per project with repos): join agent PRs (branch prefixes `claude/*`, `codex/*`, `facility/*`, configurable per project) that reached terminal state since last run → upsert outcomes rows {fate, review_rounds (submitted reviews count), fixup_commits (commits authored by non-agent after first agent commit), hours_to_terminal}. Compute rolling acceptance = merged/(merged+closed), one-shot = merged with 0 change-requests AND 0 fixups. Optional per-project `WATCHTOWER_WEBHOOK_URL`-style sink: POST summary JSON to integrations of kind webhook flagged `outcomes_sink` (compat with the v0.2 variable).
...
`analytics.rollup` (hourly): upsert analytics_daily {project, day, agent_def, model} ← runs (count/status), llm_requests (tokens, cost_cents), outcomes (acceptance/one-shot when day closes). Endpoint `GET /v1/analytics?projectId=&from=&to=&groupBy=day|agent|model` reading rollups (never raw scans on request path). Org overview endpoint `GET /v1/analytics/overview` → the numbers the web overview/analytics pages consume (live agents, spend MTD, acceptance 30d, one-shot 30d, per-project rows).

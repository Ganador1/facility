...
**Forwarding**: stream request body through untouched (do NOT buffer whole bodies; cap at 20MB); inject real provider credential (sealed → opened at boot into memory only, per provider config; refresh on change notification poll 60s); strip the virtual key header; add `anthropic-version` passthrough. Response: stream back verbatim (SSE and non-SSE), tee-parsing usage:
...
Streaming is genuinely zero-copy (no full-body buffering on the happy path — I will read the pump code); provider keys only ever in gateway memory; error envelopes match each provider's SDK expectations; metering failures never break the call; budget check is one indexed query + cache, not N; no route besides the two provider surfaces + health.

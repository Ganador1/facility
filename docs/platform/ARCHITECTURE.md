...
Facility Platform is a **control plane**. Execution — code, tests, agent runs — happens in GitHub repos, CI, and isolated sandboxes. The platform owns what must be centralized to govern an AI SDLC: identity, policy, money, knowledge, telemetry, and the human gates. Two consequences drive everything below:

1. **Compat lane first.** tam-os (tenant #1) already runs the SDLC via vendored GitHub workflows. The platform must add value without breaking that: repos keep working with their vendored assets while the platform takes over keys (gateway), telemetry (receipts sink), assets (kickstart/upgrade/fingerprints), and adds platform-native runs (PO agent, learning mode, sandboxed crew). Migration is adoption, not rewrite.
...
| 13 | Dual-lane execution: repo-native (compat) + platform-native (sandboxed) | tam-os keeps its vendored workflows day one; platform-native lane runs PO/learning/crew in platform sandboxes. Cutover per-trigger, human-gated. |
...
| 21 | Receipt integrity: platform-owned runner supervises telemetry | tam-os fetches its collector digest-pinned from the default branch so agents can't tamper with their own telemetry; in platform-native runs the runner process (outside the agent's write reach) owns receipts, preserving the same property structurally. |
...
- **Registry**: `registry_items` (kind: skill|rule|agent_contract|harness|guard|module|template_set; scope org or project or bundled), `registry_versions` (content, content_hash, status draft|active|deprecated, created_by), `bundles` (recommended sets). Bundled v1 = v0.2 crew + modules + prompts.
- **Agents & execution**: `agent_defs` (name, engine claude-code|codex|byo, model tier, contract → registry, harness → registry, triggers jsonb, sandbox_profile_id, permission set), `sandbox_profiles` (image, deps, provision_cmd, env refs, resources, driver), `runs` (agent_def, trigger, status queued→provisioning→running→awaiting_human→succeeded|failed|canceled, receipt: tokens_in/out, cost_cents, duration_ms; gh_issue/pr refs), `run_events` (structured stream: tool calls, checkpoints, logs → object storage for bulk), `steer_messages` (session steering log).
- **Money**: `provider_credentials` (org, provider, sealed secret), `virtual_keys` (project/run scope, hash, budget refs, allowed models), `llm_requests` (key, run, provider, model, tokens, cost_cents, latency, status, envelope URIs; monthly partitions), `budgets` (scope org|project|agent_def, period, limit_cents, mode soft|hard), `spend_counters` (fast-path cache for gateway enforcement).
...
**Platform-native run**: trigger (slash command webhook, schedule, manual, MCP) → `runs` row → worker picks up → driver provisions sandbox from profile → runner boots: clones repo (installation token, least scope), runs provision_cmd, launches engine CLI with contract + skills from registry, `ANTHROPIC_BASE_URL`/`OPENAI_BASE_URL` pointed at gateway with run-scoped virtual key → events stream to api (SSE fan-out to web; transcript JSONL to object storage) → agent finishes → receipt finalized (gateway aggregates), outcome hooks (PR opened, comment posted) via App identity → sandbox destroyed. Stuck session: engineer opens session view, sees live transcript, sends steer message → runner injects into engine (or restarts turn with appended instruction) — all steering audited.
...
**Learning mode**: nightly cron per enabled project → sandbox run with learning harness: reads the day's runs/reviews/failures (via api, read-only key) → drafts proposals (new skill/rule/KB entries/guard candidates — the ratchet) → each proposal = HITL item with diff preview → human approves ⇒ registry version created (draft→active) / KB entry committed; rejects ⇒ recorded, feeds next night's context.
...
- **AuthZ**: permission catalog enforced in one middleware; deny-by-default; route declares required permission; resource loaders re-check scope. Agents/runs get least-privilege machine principals (e.g., PO agent: `kb:write`, `tasks:write`, nothing else).
- **Secrets**: sealed-box at rest; decrypted only in the service that needs them (gateway for provider keys, api for App key); never returned by any API; audit on every access.
...
Single-region; no microservice mesh (3 services); no Kafka (PG outbox + pg-boss); no OPA (permission strings); no bespoke metrics stack (analytics from our own tables; optional webhook sinks); no plugin marketplace (registry + bundles suffice); Codex/BYO engines run through the same runner protocol rather than bespoke integrations.

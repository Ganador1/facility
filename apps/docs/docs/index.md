...
| **execution** | agents run in disposable sandboxes; sessions stream live; a stuck agent can be steered by a human, on the record |
| **knowledge** | skills, rules, contracts, harnesses, guards — versioned, bundled, immutable once published |
| **humans** | one inbox for every decision an agent needs: plan gates, learning validations, budget overrides |
...
cannot push to protected branches. A person accepts the plan; a person signs
the merge. The platform makes those decisions cheap to exercise — it never
makes them optional.

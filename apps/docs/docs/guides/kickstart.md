...
4. **Preview.** Every file the kickstart will write, with hashes, and the
...
5. **Open the PR.** The platform commits the rendered assets to
   ...
   default branch, confirm App permissions.
6. **Merge it.** That's gate 2 muscle memory from day zero. On merge the
